...
# Create the stage
filename = 'MoonStage.usda'
stage = Usd.Stage.CreateNew(filename)
UsdGeom.SetStageUpAxis(stage, UsdGeom.Tokens.z)

# Create parent prim
planeroot = UsdGeom.Xform.Define(stage, "/Moon")
planeroot.AddScaleOp().Set(Gf.Vec3f(100, 100, 100)) #scale
Usd.ModelAPI(planeroot).SetKind(Kind.Tokens.component)
stage.SetDefaultPrim(planeroot.GetPrim())
...
height_scale = .025
subdiv_lat = int(4096) # latitude divisions
subdiv_lon = int(2048)  # longitude divisions
planeroot.AddTranslateOp().Set(Gf.Vec3f(0.0, 0.0, 0.0)) #z=radius to get moon above mesh
...
ColorShader.CreateInput("specular", Sdf.ValueTypeNames.Float).Set(0.1)
ColorShader.CreateInput("clearcoat", Sdf.ValueTypeNames.Float).Set(0.05)
...
#normal map for microdetails
normalTextureSampler = UsdShade.Shader.Define(stage, '/Moon/lunarcolor/normalTexture')
normalTextureSampler.CreateIdAttr('UsdUVTexture')
normalTextureSampler.CreateInput('file', Sdf.ValueTypeNames.Asset).Set(height_map)
...
ColorShader.CreateInput('normal', Sdf.ValueTypeNames.Normal3f).ConnectToSource(normalTextureSampler.ConnectableAPI(), 'rgb')
