...
def load_lightglue(crop1, crop2):
  ...
  lg_sim_score = lightglue_geometric_similarity(mkpts0, mkpts1, scores, visFig=VISUALIZE_FIG, matching_name=MATCHING_NAME)
  return lg_sim_score
...
if __name__ == "__main__":
  ...
  print(f"partsbased_sim_score: {part_simscore}")
  
  lg_sim_score = load_lightglue(crop_ref, crop_img)
  print(f"lightglue_sim_score: {lg_sim_score}")
  
  hu_sim_score = load_hu(crop_ref, crop_img, Y)
  print(f"hu_sim_score: {hu_sim_score}")
  
      
  total_simularity_score = A*part_simscore + B*lg_sim_score + C*hu_sim_score
  print(total_simularity_score)
