...
from extract_segmentations import extract_segmentations
from parquetToCSV import convert_to_csv

"""
    Taylor J. Brown | 13Mar2025 | ISL-Inteligent Systems Lab | Waymo Open Dataset Extraction Script
    ...
    The image and segmentation files are downloaded from the following links:
...
def main():
    ...
    # Converts parquet files to CSV (This makes the parquet files human readable)
    ...
    extract_segmentations(segmentation_file, output_dir="extracted_segmentations")
    
    # Print number of images and segmentation masks extracted
    print(f"\nNumber of images extracted: {len(os.listdir('extracted_images'))}")
    print(f"Number of segmentation masks extracted: {len(os.listdir('extracted_segmentations'))}")
