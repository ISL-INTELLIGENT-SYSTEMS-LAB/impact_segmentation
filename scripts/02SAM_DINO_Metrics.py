#!/usr/bin/env python3
"""
SAM -> DINO multi-reference ReID pipeline.

Goal:
- Match the evaluation style of the user's CSV-driven DINO baseline:
    * Every object in every image is treated as a reference
    * That reference is compared against every other image
    * Per-reference and overall metrics are computed

Key rule:
- SAM provides the boxes used for DINO embeddings
- GT CSV is NEVER used for cropping or feature extraction
- GT CSV is used ONLY in the evaluation phase:
    1) detection metrics
    2) post-hoc identity assignment to SAM predictions
    3) retrieval metric scoring
...
- Overall Retrieval mAP
- Per-reference Top-1
- Per-reference Retrieval mAP
...
def match_predictions_to_gt(pred_boxes, gt_items, iou_thresh=0.5):
    """
    Greedy one-to-one matching by IoU.
    Returns a list, same length as pred_boxes:
      [{'label_name': str, 'gt_index': int, 'iou': float} or None]
    """
...
def draw_boxes_on_image(image_path, items, output_path, highlight_index=None):
    ...
    for idx, item in enumerate(items):
        ...
        color = (0, 255, 0) if idx == highlight_index else (255, 255, 0)
...
def write_markdown_report(path, summary, reference_rows):
    ...
    lines = []
    lines.append("# SAM -> DINO Multi-Reference Metrics Report")
    lines.append("")
...
def get_feature_for_pred_box(extractor, image_path, pred_box, device):
    pil = load_pil_image(image_path)
    orig_w, orig_h = pil.size
    img_tensor = load_image_as_tensor(image_path).to(device)

    scaled_box = scale_box_to_224(pred_box, orig_w, orig_h)
...
def main():
    parser = argparse.ArgumentParser(description="SAM->DINO multi-reference ReID pipeline")
    ...
    parser.add_argument("--output", default="output_sam_dino_multiref", help="Output directory")
    ...
    parser.add_argument("--sam-threshold", type=float, default=0.5, help="SAM score threshold")
    ...
    for image_path in image_paths:
        ...
        pred_boxes, pred_scores = sam_predictor.predict(image_path)
        ...
        for pred_idx, pred_box in enumerate(pred_boxes):
            feat = get_feature_for_pred_box(extractor, image_path, pred_box, device)
            pred_records.append({
                "pred_idx": pred_idx,
                "bbox_xyxy": pred_box,
    ...
    for image_name, pred_items in pred_records_by_image.items():
        for pred_item in pred_items:
            # No GT-based filtering: all predictions become references
    ...
    for ref_num, ref_data in enumerate(all_references, start=1):
        ...
        ref_matched = ref_item["matched_info"]
        ref_label = ref_matched["label_name"] if ref_matched is not None else "unmatched"
        ref_feat = ref_item["embedding"]
        ...
        for query_image in query_names:
            ...
            for item in query_pred_items:
                # Include all predictions; label from GT if matched, else "unmatched"
                query_feats.append(item["embedding"])
                if item["matched_info"] is None:
                    query_labels.append("unmatched")
                else:
            ...
            draw_pred_boxes_on_image(
                ...
                [r["bbox_xyxy"] for r in query_pred_items],
                [r["matched_info"] for r in query_pred_items],
                query_vis_path,
            ...
            if gt_indices:
                ...
                # Keep same behavior as baseline: first GT index for multiclass top1
                ...
                query_ap = float(np.mean(precisions)) if precisions else 0.0
