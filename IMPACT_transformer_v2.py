    ...
    def __init__(self, root_dir, img_dir=None, seg_dir=None, imgFrag=None, segFrag=None, imgfix=".png", segfix=".png"):
        ...
        for f in sorted(os.listdir(img_path)):
            ...
            if filetype.is_image(file_path):
                ...
                #print(os.path.exists(seg_file_path))
...
def main():
    ...
    PROJECTDIR = 'IMPACT'
    subproject = 'ai4mars'
    DATADIR = f'/home/{user}/ai4mars-dataset-merged-0.1/re_encoded_files'
    ...
    location_default_model = './default_model'
    ...
    config = MaskFormerConfig.from_pretrained(os.path.join(location_default_model, config_name))
    ...
    model = MaskFormerForInstanceSegmentation(os.path.join(location_default_model, MODEL_NAME), config=config, local_files_only=True)
    ...
    # Cannot get the maskformer config. have to have a saved copy and use local files
    #config = MaskFormerConfig.from_pretrained(os.path.join(location_default_model, config_name))
    #print("[INFO] displaying the MaskFormer configuration...")

     
    # We should already have a model with the pretrained weights saved in the default_model location
    model = MaskFormerForInstanceSegmentation(os.path.join(location_default_model, MODEL_NAME))
